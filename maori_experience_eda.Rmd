...
```
People who travel alone is less likely to experience Maori cultural activity, while people who travel with other adult (including partner and child over 15) is more likely to experience Maori cultural activity. Later we will group this into:
1. Travel alone (baseline)
2. Travel with child under 15 (OHE-column)
3. Travel with other adult (OHE-column)
...
```{r}
cols = c("response_id", "year", "qtr", "country_of_residence_group", "gender", "first_nz_trip", "purpose_of_visit_main", "purpose_subtype", "no_days_in_nz", "age_range", "visited_ni", "visited_si", "main_transport_type")
...
```{r}
smh_df <- smh_df %>% 
  mutate(visited_ni = as.integer(visited_ni),
         visited_si = as.integer(visited_si))
```
...
smh_df[both_zero,]
```
We don't expect a row to have null value for both visited_ni and visited_si, because they should visit at least north or south island. Maybe the respondent did not answer the question, so we will replace them with NA.
...
transport_methods %>% 
  filter(response_id %in% null_response_id)
```
...
## TODO
Explore these columns to see if they have association with participation in Maori cultural activity: gender, country_of_residence_group, first_nz_trip, purpose_of_visit_main, purpose_subtype, no_days_in_nz, age_range, visited_ni, visited_si, main_transport_type

If they are important, do some feature engineering (e.g. grouping, one hot encoding, etc.) to make them suitable for modelling.